...
resource "azurerm_public_ip" "vmss" {
  name                    = format("%s-pip-%s", var.namespace, var.service_name)
...
resource "azurerm_linux_virtual_machine_scale_set" "vmss" {
  name                = format("%s-vmss-%s", var.namespace, var.service_name)
  resource_group_name = var.resource_group_name
  location            = var.location
  ...
  network_interface {
    name    = format("%s-nic-%s", var.namespace, var.service_name)
    primary = true
