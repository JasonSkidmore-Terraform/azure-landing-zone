...
resource "azurerm_postgresql_server" "postgres" {
  ...
  name                = lower(format("%s-postgresql-%s", var.namespace, random_pet.postgres-name.id))
...
resource "azurerm_postgresql_virtual_network_rule" "postgres" {
  ...
  name                = format("%s-pgsql-vnet-rule", var.namespace)
...
resource "azurerm_postgresql_firewall_rule" "postgres" {
  ...
  name                = format("%s-pgsql-public-%s", var.namespace, count.index)
